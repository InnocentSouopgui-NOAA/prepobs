...
#                            specific parm settings.
#     $HOMEprepobs         - formerly $HOMEobsproc_prep prior to WCOSS2
#                            string indicating directory path to prep
#                            subdirectories (fix/, sorc/, ush/) under version
#                            control (pre-WCOSS2, in production this is normally
#                            ${NWROOT}/obsproc_prep.vX.Y.Z where X.Y.Z is
#                            version number being used, usually the latest)
#                            WCOSS2: prepobs module 
...
#                            version number being used, usually the latest)
#                            WCOSS2: $PACKAGEROOT/obsproc.vX.Y.Z
#
...
#                   files
#                   Default is "${HOMEprepobs}/fix"
#     DICTPREP      String indicating directory path for PREPOBS dictionary
#                   files
#                   Default is "${OPSROOT}/decoders/decod_shared/dictionaries"
...
#                   Default is "$DCOMROOT", where default for $DCOMROOT
#                   is "/lfs/h1/ops/prod/dcom"
#
...
USHSYND=${USHSYND:-${HOMEprepobs}/ush}
USHGLERL=${USHGLERL:-${HOMEprepobs}/ush}
USHPREV=${USHPREV:-${HOMEprepobs}/ush}
USHCQC=${USHCQC:-${HOMEprepobs}/ush}
USHPQC=${USHPQC:-${HOMEprepobs}/ush}
USHVQC=${USHVQC:-${HOMEprepobs}/ush}
USHAQC=${USHAQC:-${HOMEprepobs}/ush}
USHOIQC=${USHOIQC:-${HOMEprepobs}/ush}

EXECPREP=${EXECPREP:-${HOMEprepobs}/exec}
PARMPREP=${PARMPREP:-${HOMEobsproc}/parm}
FIXPREP=${FIXPREP:-${HOMEprepobs}/fix}
DICTPREP=${DICTPREP:-${OPSROOT}/decoders/decod_shared/dictionaries}

EXECSYND=${EXECSYND:-${HOMEprepobs}/exec}
PARMSYND=${PARMSYND:-${HOMEobsproc}/parm}
FIXSYND=${FIXSYND:-${HOMEprepobs}/fix}
