...
C 2021-04-22  C. Hill / J. Woollen / J. Ator --
C       The BUFR library with dynamic memory allocation is invoked,
C       and the BUFR parameters MAXSS and MXMSGL are each increased to
C       600000 to accommodate the initial read of high-resolution
C       radiosonde/dropsonde data from the 'uprair' dump files.
C       ISETPRM calls are placed before the first BUFR library call,
C       to SETBMISS.
...
C     DUMP FILES, CHANGES WERE NEEDED. NUMGPS WAS ADDED AS A WAY TO SAVE
C     TIME AND LOCATION DISPLACEMENT DATA. MAXOBS WAS INCREASED TO 4500
C     DUE TO SEGFAULTS WHEN MAXOBS WAS SET TO 3500. 
C
C
C USAGE:
C   INPUT FILES:
...
C     UNIT EE+7 - 'UPRAIR' BUFR DATA DUMP FILE (BUFR RAOB/PIBAL/DROPS)
...
C                 WIND INFORMATION)
C     UNIT FF+9 - 'SALDRN' BUFR DATA DUMP FILE (MARINE SURFACE)
C
...
C  VALUES IN THE IQ (EQUIV. TO DAT) & JQ (EQUIV. TO TMP) ARRAYS (U-AIR)
      PARAMETER (NUMVAR = 6, NUMQMS = 5, NUMGPS = 3)
C      PARAMETER (NUMVAR = 9, NUMQMS = 5)
C PARAMETER NAME "NUMDAT" THROUGHOUT PGM SETS SIZE OF DAT AND TMP ARRAYS
      PARAMETER (NUMDAT = NUMVAR + (2 * NUMQMS) + NUMGPS)
C      PARAMETER (NUMDAT = NUMVAR + (2 * NUMQMS))
...
C  IN INTERFACE WITH SUBROUTINE IW3UNPBF
     ...
     $ SUBSKP,RASS,WRMISS,SKGP45,npkrpt,SKGNSS,UPRAIR
     ...
     $ JSURFM(13),JSURFW(13),FWINDO(13),PFRALT,npkrpt(13)
     ...
     $ IVADFL,IVADSP,FLDMGS,FLDMAF,FLDMFR,DROPSN,DWINDO,UPRAIR
      COMMON/PARM4/ICODE
     ...
     $ FLDMFR,WRMISS,SKGP45,JPASCD,IAWNDO,npkrpt,SKGNSS,UPRAIR
      NAMELIST/PARM/IUNIT
...
C  -------------------------------------------------------------------
      ...
      CALL ISETPRM ( 'MXMSGL', 600000 )  ! CH 04/15/21, moved 04/22/21
      CALL ISETPRM ( 'MAXSS',  600000 )  ! CH 04/15/21, moved 04/22/21
CCC                                     above lines moved to here from
CCC                                     8 places below at suggestion
CCC                                     of J. Wollen
ccccc CALL SETBMISS(10E10_8)
      CALL SETBMISS(10E8_8)
      BMISS=GETBMISS()
      CALL MAXOUT(50000)                 ! CH 08/09/21
      print'(1X)'
...
C  LEVELS THAT CAN BE PROCESSED AND ENCODED INTO BUFR MESSAGES
      ...
      CHARACTER*8   STNPRT,STNID,FILNAM(8),DSNAME,SUBSET_d,subset_t
     ...
     $ GOESRD,FLRECO,DROPSN,SUBSKP,PROFILERinADPUPA,RASS,UPRAIR
     ...
     $ obs8_8(2)
      ...
      DATA  FILNAM/'UPRAIR  ','AIRCAR  ','AIRCFT  ','SATWND  ',
     $             'PROFLR  ','VADWND  ','RASSDA  ','ADPUPA  '/
...
C***********************************************************************
 2100 CONTINUE
      IF(NN.LT.5.OR.(NN.EQ.5.AND.IPRFPIB.GT.0).OR.NN.EQ.8)  THEN
C***********************************************************************
...
C IFLAG = 1 RETURNS DATA SET INFO (ONLY) AFTER FIRST CALL
 ...
 8872          FORMAT(5X,A47,6F6.0,' TO ',6F6.0)
               PRINT'(1X)'
            ELSE IF(NN.EQ.1.OR.NN.EQ.8)  THEN
...
C CHECK FOR VALID DATA, IF NONE UNPACK NEXT REPORT
     ...
     $       IDATA(31),IDATA(35),IDATA(37),IDATA(45)).EQ.0)  GO TO 2100
C.......................................................................
 ...
 7003 CONTINUE
  ...
  950 FORMAT(19X,'===>  A TOTAL OF ',I6,'  REPORTS WERE UNPACKED ',
     $ 'FROM THIS FILE, WHETHER KEPT OR SKIPPED  <===',/)
      IF((NN.EQ.1.OR.NN.EQ.8).AND.KOUNT.EQ.0)  ICODE = 4
      KNTUUU(NN) = KOUNT
...
C  REPORTS HERE)
      IF(NFILE.EQ.IUNIT(5).OR.NFILE.EQ.IUNIT(5)+7)  THEN
...
C     DUMP FILES, CHANGES WERE NEEDED. NUMGPS WAS ADDED AS A WAY TO SAVE
C     TIME AND LOCATION DISPLACEMENT DATA. MAXOBS WAS INCREASED TO 4500
C     DUE TO SEGFAULTS WHEN MAXOBS WAS SET TO 3500. HP, XP, AND YP WERE
C     ADDED TO KEEP LOCATION AND TIME INFORMATION. IS WAS ADDED AS AN
C     ARRAY TO KEEP THE DATA. UPRAIR DATA USES DATA LEVEL CATEGORIES 1,
C     2, AND 10, SO GPS DATA WAS ADDED TO ALL 3. THE SIZE OF EACH DATA
C     ARRAY NEEDED TO BE INCREASED BY 3. 
C
C USAGE:    CALL GETUPA(*,*)
C   OUTPUT FILES:
C     UNIT 06  - STANDARD OUTPUT PRINT
...
C DATA AT PRESSURE LEVELS (MESHED SOUNDING)
      ...
      EQUIVALENCE  (DAT(1,NUMVAR+NUMQMS+NUMQMS+1),IS)
      DATA  XMISS/99999./,YMISS/99998.8/,IMISS/99999/
...
C***********************************************************************
C      COLLECT UPPER-AIR DATA IN DATA LEVEL CATEGORIES 1 - 5, 10, 15
...
C  GPS VALUES:
C   DAT(NL,17) - HRDR   DAT(NL,18) - YDR   DAT(NL,19) - XDR
C
C  # - SENSIBLE TEMPERATURE FOR ALL NON-SATELLITE DATA TYPES
C      VIRTUAL TEMPERATURE FOR RTOVS OR ATOVS (UNPACKED NMCEDS)
C
C NL IS COUNT OF TOTAL LVLS FROM ALL CATEGORIES (1-5, 10, 11, 15 + SPECIAL)
C NP3 IS COUNT OF ALL LEVELS CONTAINING VALID WIND DATA IN CAT. 1,3,5,10
C NPZ IS COUNT OF ALL LVLS CONTAINING NON-MISSING HGHT DATA IN
C CATEGORIES 1,2,10
...
C  GET GPS DATA IF AVAILABLE
         IF(RDATA(II+11).LT.BMISS) THEN
           GPST=NINT(RDATA(II+11))
         ELSE 
           GPST=BMISS
         END IF
         IF(RDATA(II+12).LT.BMISS) THEN
           GPSY=(NINT(RDATA(II+12))/100000.)
         ELSE
           GPSY=BMISS
         END IF
         IF(RDATA(II+13).LT.BMISS) THEN
           GPSX=(NINT(RDATA(II+13))/100000.)
         ELSE
           GPSX=BMISS
         END IF
         IBTEST = 0
...
C     THERE IS A CHANGE IN NUMBER OF VARIABLES SAVED. SAME WITH THE 10
C     FIVE LINES BELOW.
      II = IDATA(16) - 10
      NP1VLD = 0
...
C GET PREPBUFR TBL VALUES FOR PRESS., TEMP., AND MOISTURE AT THIS LEVEL
         ...
         IQM = NINT(RDATA(II+5))
C SAVE GPS DATA
         IF(RDATA(II+7).LT.BMISS) THEN
           GPST = NINT(RDATA(II+7))
         ELSE
           GPST=BMISS
         ENDIF
         IF(RDATA(II+8).LT.BMISS) THEN
           GPSY = NINT(RDATA(II+8))
         ELSE
           GPSY=BMISS
         ENDIF
         IF(RDATA(II+9).LT.BMISS) THEN
           GPSX = NINT(RDATA(II+9))
         ELSE
           GPSX=BMISS
         ENDIF
...
C N125 IS THE NUMBER OF CAT. 1, 2, AND 5 LEVELS COLLECTED
      N125 = NL
C---------------------------------------------------vv--CH 07/23/2021-vv
C-----------------------------------------------------------------------
C    BUFR SPECIFIC LEVEL MASS AND WIND DATA (DATA LEVEL CATEGORY 10)
C-----------------------------------------------------------------------
C
C NN IS NO. OF LEVELS THAT WILL BE CONSIDERED
C NB IS NO. OF BUFR DATA LEVELS (CAT. 10) ACTUALLY COLLECTED
      NB = 0
      NN = IDATA(31)
C      II = IDATA(32) - 11
      II = IDATA(32) - 14
...
C GET PREPBUFR TABLE VALUES FOR PRESSURE, HGHT, TEMP., MOISTURE, & WIND
         ...
         END IF
         IF(RDATA(II+12).LT.BMISS) THEN
           GPSY=(NINT(RDATA(II+12))*0.00001)
         ELSE
           GPSY=BMISS
         ENDIF
         IF(RDATA(II+13).LT.BMISS) THEN
           GPSX=(NINT(RDATA(II+13))*0.00001)
         ELSE
           GPSX=BMISS
         ENDIF
         IBTEST = 0
...
C PROCESS NON-MSG WIND REGARDLESS OF PREPBUFR TABLE VALUE
         ...
         IS(NL,3) = GPSX 
      ENDDO
C RESET IDATA(31) TO NUMBER OF 'GOOD' CAT. 10 LEVELS ACTUALLY PROCESSED
      IDATA(31) = NB
 5695 CONTINUE
C N125_10 IS THE NUMBER OF CAT. 1, 2, 5, AND 10 LEVELS COLLECTED
      N125_10 = NL
C---------------------------------------------------^^--CH 07/23/2021-^^
C-----------------------------------------------------------------------
...
C IF PRESSURE > STN (SFC) PRESSURE (GOOD ONLY) FLAG ALL DATA ON THIS LVL
         IF(NP1VLD.GE.1.AND.
     +     (NINT(RDATA(II))*0.1).GT.DAT(N125_10+1,1)) THEN
          ! this should not be monitored by GSI!! (but is since qm=15)
            IPM = 15
            IVM = 15
CVVVVV%%%%%TEMP FOR PRINT
...
C   AS SUCH
      IF(NP1VLD.NE.0)  THEN
         NCN = NM + 1
         IF(IABS(NP1VLD).EQ.2)  NCN = N125_10 + 1
         STAP = DAT(NCN,1)
         DAT(NCN,2) = ELEV
         NPZ = NPZ + 1
...
C  PRESSURE
      CALL SMERGE(NM,NSIG,N5,NB,NP,NH,N15,NLEV,IPRT,STOP)
      IF(NLEV.LE.0)  THEN
C IF NLEV IS ZERO, SKIP REPORT AND PRINT DIAGNOSTIC
 ...
 1903 FORMAT(//,17X,'>>> COMPLETE LISTING OF DATA FOR STATION ID ',A8,
  ...
  899 FORMAT(2X,'     UPPER-',4X,'PRESSURE',6X,'HEIGHT',4X,
     ...
     $    IR(N,2),IQ(N,4),IR(N,4),IQ(N,5),IR(N,5),IS(N,1),
     $    IS(N,2),IS(N,3),N=1,NLEV)
...
C 2021-07-23 C. Hill -- NB added to represent new data level category 10
C 2023-01-31  N. ESPOSITO -- WITH THE ADDITION OF GPS DATA TO UPRAIR
C     DUMP FILES, CHANGES WERE NEEDED. NUMGPS WAS ADDED AS A WAY TO SAVE
C     TIME AND LOCATION DISPLACEMENT DATA. MAXOBS WAS INCREASED TO 4500
C     DUE TO SEGFAULTS WHEN MAXOBS WAS SET TO 3500. HP, XP, AND YP WERE
C     ADDED TO KEEP LOCATION AND TIME INFORMATION. IS WAS ADDED AS AN
C     ARRAY TO KEEP THE DATA.
C
C USAGE:    CALL SMERGE(NM,NSIG,N5,NB,NP,NH,N15,NLEV,IPRT,STOP)
C   INPUT ARGUMENT LIST:
C     NM       - NO. OF LEVELS IN CAT.  1 (MANDATORY MASS/WIND)
C     NSIG     - NO. OF LEVELS IN CAT.  2 (SIG. MASS)
C     N5       - NO. OF LEVELS IN CAT.  5 (TROPOPAUSE)
C     NB       - NO. OF LEVELS IN CAT. 10 (BUFR MASS/WIND)
...
C
C$$$
      SUBROUTINE SMERGE(NM,NSIG,N5,NB,NP,NH,N15,NLEV,IPRT,STOP)
      PARAMETER (MXWRDH = 15)
      PARAMETER (MXLVL = 600)
      PARAMETER (NUMVAR = 6, NUMQMS = 5, NUMGPS = 3)
C      PARAMETER (NUMVAR = 9, NUMQMS = 5)
C      PARAMETER (NUMDAT = NUMVAR + (2 * NUMQMS))
     ...
     $ JR(MXLVL,NUMQMS)
      REAL  PLIMIT(4),PRS(3),IS(MXLVL,NUMGPS),JS(MXLVL,NUMGPS)
      real(8)  BMISS,alon_8,alat_8
      COMMON/WORD/LWI,LWR
      COMMON/RPTHDR/SUBSET_d,STNID,HDR(2:MXWRDH),alon_8,alat_8
      COMMON/ADP/ISATOB,PMAND(23),RDATA(MAXOBS),IPRINT
      COMMON/SNDATA/DAT(MXLVL,NUMDAT)
      ...
      EQUIVALENCE(DAT(1,NUMVAR+NUMQMS+1+NUMGPS),TEST)
      EQUIVALENCE(DAT(1,NUMVAR+NUMQMS+1),IR)
      EQUIVALENCE(TMP(1,NUMVAR+NUMQMS+1),JR)
      EQUIVALENCE  (DAT(1,NUMVAR+NUMQMS+NUMQMS+1),IS) 
      EQUIVALENCE  (TMP(1,NUMVAR+NUMQMS+NUMQMS+1),JS) 
...
C NS IS NUMBER OF CAT. 2 AND CAT. 5 LEVELS COMBINED
      NS = NSIG + N5
      NSN = NS
C KLVL IS NUMBER OF CAT. 1, 2, 5, AND 10 LEVELS COMBINED
      KLVL = NM + NS + NB
      NSTZ = NM + NS
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C DETAILED PRINT FOR SPECIFIED REPORT(S) -- IF ENTERED IN DATA CARDS
...
C COMPARE CAT. 2 PRESS. W/ THEMSELVES & W/ CAT. 5 PRESS.; LOOK FOR DUPL.
      NST = NM + 1
C  KLVL REPLACED BELOW WITH NSTZ, DEFINED ABOVE   CH 07/23/2021
      LOOP2: DO N = NST,NSTZ
         IP = NINT(DAT(N,1)*SCALE)
         LOOP2n1: DO M = NST,NSTZ
...
C DETAILED PRINT FOR SPECIFIED REPORT(S) -- IF ENTERED IN DATA CARDS
      IF(IPRT(2).EQ.1.AND.NUMVAR.LT.11.AND.NUMQMS.LT.9)  THEN
         PRINT 903, STNID
  903 FORMAT(/' SMERGE: ',A8,', CAT. 1,2,5,10 LVLS W/ DUPL. ZEROED OUT')
         IF(KLVL.GT.0)  THEN
            DO N = 1,KLVL
               PRINT 900, (DAT(N,K),K=1,NUMVAR)
               PRINT 905, (IQ(N,L),L=1,NUMQMS)
...
C  OR 50MB LVLS
      NSTA = NM + NS + NB + NP + NH + N15
C IF MODPRT SMALL, STD. O-PUT PRINT NO. OF LVL IN EACH DATA LEVEL
C  CATEGORY. FOR ALL RPTS
      IF(MODPRT.LE.300.AND.MODPRT.GE.0)
     $ PRINT 9090,STNID,IDATA(9),NSTA,NM,NSIG,N5,NB,NP,NH,N15,
     $ REAL(LASTP)/SCALE
 9090 FORMAT(1X,'ID ',A8,' DUMP RT.=',I3,', NLVLS: TOT=',I2,', CAT 1='
     $,I2,', CAT 2=',I2,', CAT 5=',I2,', CAT 10=',I3,
     $', CAT 3+SPEC LVLS=',I2,', CAT 4=',I2,', CAT 15=',I2,
     $' - TOP SIG P(W/MASS)=',F6.1,'MB')
      IF(IPRT(1).EQ.1.OR.IPRT(2).EQ.1)  THEN
         PRINT 1902, STNID
...
C                SIGNIFICANT (DATA LEVEL CAT. 2,5) AND
C                BUFR (DATA LEVEL CAT. 10)  LEVEL DATA
C***********************************************************************
C
...
C$$$
     ...
     $ HP(MXLVL),XP(MXLVL),YP(MXLVL)
      REAL(8)  BMISS,obs8_8,alon_8,alat_8,IS(MXLVL,NUMGPS)
      COMMON/STRCTY/ICAT(MXLVL)
      COMMON/STRMOB/MOBS(MXTYPV,MXWRDL,MXLVL)
      COMMON/UNITNO/NFILE,IUNIT(28)
      COMMON/ADP/ISATOB,PMAND(23),RDATA(MAXOBS),IPRINT
...
C SET REPORT SEQ. NUMBER TO "ISQNUM_UPA" AND STORE IN HDR(11) IF A
C  RADIOSONDE, PIBAL, DROPWINSONDE OR CLASS REPORT IN ADPUPA FILE
      IF(NFILE.EQ.IUNIT(5).OR.NFILE.EQ.IUNIT(5)+7) HDR(11) = ISQNUM_UPA
      CALL FILLX(LL,IERF)
      LL_WIND = LL
...
C 2023-01-31  N. ESPOSITO -- WITH THE ADDITON OF GPS DATA, WE NEEDED TO
C             NEW UPRAIR BUFR DUMPS HAVE GPS DATA IN THEM, ALLOWING US
C             TO USE THAT DATA INSTEAD OF CALCULATING DRIFT USING WIND
C             SPEEDS. GPSFLAG WAS ADDED TO DICTATE WHEN WE HAVE THE
C             DATA. GPSFLAG(1) IS SET TO 1 WHEN WE USE TIME DISPLACEMENT
C             DATA FROM THE GPS INSTEAD OF CALCULATING IT. GPSFLAG(2) IS
C             SET TO 1 WHEN WE USE LOCATION DISPLACEMENT DATA INSTEAD OF
C             CALCULATING IT USING THE WIND DIRECTION DATA. IF THE FLAGS
C             ARE KEPT AT 0, THEN WE CONTINUE CALCULATING THE WAY WE
C             USED TO.
C
...
C$$$
     ...
     $ TDRIFT_LL(MXLVL),KFLAG(5),LFLAG(3),GPSFLAG(2),PLO,IFLTIM,JJ,II
      EQUIVALENCE  (DAT,PP),(DAT(1,2),ZP),(DAT(1,3),TP),(DAT(1,4),DP),
...
C            STANDARD ATMOSPHERE)}
     ...
     $    "proceed w/ drift time calc.")', I,PP(I)
         ...
         ZDRIFT(JJ) = ZP(I)
         IF(JJ.GT.1)  THEN
            IF((HP(I).LT.BMISS).AND.(HP(I).NE.XMISS)) THEN
...
C    ALL OF THE ORIGINAL, NORMAL CHECKS.
               TDRIFT(JJ)=HP(I)
               PRINT *, "TDRIFT FROM GPS: ", STNID, TDRIFT(JJ)
               GPSFLAG(1) = 1
            ELSE
               TDRIFT(JJ) = TDRIFT(JJ-1) +
     $          (ZDRIFT(JJ) - ZDRIFT(JJ-1))/ASCRT
               PRINT *, "TDRIFT FROM WIND: ", STNID,TDRIFT(JJ)
               GPSFLAG(1) = 0
            ENDIF
...
C     PREVIOUS LEVEL TIME VALUE
     ...
     $          "previous level drift time value")'
               JJ = JJ - 1
            ELSE  IF((TDRIFT(JJ)-TDRIFT(JJ-1).LT.0.0).AND.
     $       (GPSFLAG(1).EQ.0)) THEN
...
C               AS WELL)
 ...
 8475 FORMAT(' * * BALLOON DRFT TIME BAD-ID=',A8,', LAT=',F7.2,'N, ',
     ...
     $ ' LVLS ABOVE')
                  ...
                  IFLTIM = 1
               END IF
            ELSE IF((ABS(TDRIFT(JJ)-TDRIFT(JJ-1)).GT.3600.).AND.
     $       (GPSFLAG(1).EQ.0)) THEN
...
C    GPSFLAG(2) AT 0 AND DO ALL OF THE ORIGINAL CHECKS.
            IF ((XP(I).LT.BMISS).AND.(XP(I).NE.XMISS).AND.
     $    (YP(I).LT.BMISS).AND.(YP(I).NE.XMISS)) THEN
              XDRIFT(II) = RDATA(2)+XP(I)
              YDRIFT(II) = RDATA(1)+YP(I)
              GPSFLAG(2) = 1
              PRINT *,"X/Y DRIFT FROM GPS: ",STNID,XDRIFT(II),YDRIFT(II)
            ELSE
     ...
     $                    (TDRIFT_LL(II) - TDRIFT_LL(II-1)) * CON)
              GPSFLAG(2) = 0
              PRINT *,"X/Y DRIFT FROM WIND: ",STNID,XDRIFT(II),
     $                     YDRIFT(II)
            END IF
...
C     (NOTE: DRIFT TIME MAY STILL BE CALCULATED DEPENDING UPON TEST)
     ...
     $          "revert back to previous level drift lat/lon value")'
               II = II - 1
            ELSE  IF((STNID.EQ.'89009   ').AND.(GPSFLAG(2).EQ.0))  THEN
...
C     (NOTE: DRIFT TIME IS STILL CALCULATED)
 ...
 9425 FORMAT(' * * BALLOON LAT/LON DRFT -ID=',A8,', LAT=',F7.2,'N, ',
     ...
     $ 'CALCULATED)')
               II = II - 1
               KFLAG(2) = 1
            ELSE  IF((ABS(XDRIFT(II)-XDRIFT(II-1)).GT.1.0).AND.(
     $             GPSFLAG(2).EQ.0)) THEN
...
C         REPORT PROCESSED FROM ADPUPA FILE IN SUBR. STOROB)
         ISQNUM(NN) = 0
         IF(NN.EQ.1.OR.NN.EQ.8)  ISQNUM(NN) = ISQNUM_UPA
         ITIMES(NN) = 1
      END IF
      IFLC6 = 0
      KLVL = IDATA(23)
...
C$$$
      ...
      PARAMETER (MAXOBS = 4500)
      PARAMETER (NUMOBS2 = 43)
      CHARACTER*8   STNID,FILNAM(6),DSNAME,SUBSET_d
      CHARACTER*41  NAME1
      LOGICAL  LFM,MARLND,MSLBOG,ATLAS,PFRALT,SFLAND,SUBSKP,PFRALT_save,
     $ npkrpt
      INTEGER  IDATA(MAXOBS),NC(6),ISQNUM(3),NOBS3_SAVE(7)
      INTEGER(8)  IDSDAT,IDSDMP_8
     ...
     $ JSURFM(13),JSURFW(13),FWINDO(13),PFRALT,npkrpt(13)
      ...
      COMMON/SKPCNT/KSKPUA,KSKPSF,KMSGUA,KMSGSF,KBDYUA,KBDYSF,KTIMUA,
     $ KTIMSF,KSKNSW,KSKREC,KNOATS,KNTUPA,KNTCAR,KNTAIR,KNTSWN,KNTPRO,
     $ KNTVAD,KNTRAS,KNTDUM(3),KNTSSS(6),KSKSWN,KSKACF,KNDCDE(3),
...
C IFLAG = 1 RETURNS DATA SET INFO (ONLY) AFTER FIRST CALL
 ...
 3872 FORMAT(42X,'MESONET COOPERATIVE NETWORK .............',F5.0/)
         ELSE  IF(NN.EQ.6)  THEN
            PRINT 3873, FWINDO(13)
 3873 FORMAT(42X,'SAILDRONES ',30('.'),F5.0/)
         END IF
         GO TO 2100
C.......................................................................
      END IF
...
C   ITYP = 13 ===> SAILDRONE
C   ITYP = 14-19 > RESERVED FOR FUTURE TYPES
...
C        13 ..... SAILDRONE VEHICLE
C        14-19 .. RESERVED FOR FUTURE TYPES
...
C          534        540        551       560 561 562 563 564  --> RTYPE
C          ---        ---        ---       --- --- --- --- ---
     $ 30,  12, 5*30,  10, 10*30,  3, 8*30, 13,  4,  4,  4,  4 /
...
C  MNEMONICS PASSED INTO SUBROUTINE "UFBINT".
     ...
     $ .FALSE.,3*.TRUE.,.FALSE.,2*.TRUE.,.FALSE.,.TRUE./,ITYPL/0/,ISUB/0/,ISUBT/0/,
     $ IRECL/0/,AERS/12*0.0/
...
c - end of CEILING processing
     ...
     $                                   NOBS3(5),IRET,'DOSW HOSW POSW')
         ...
         ELSE IF(SUBSET(ITYP).EQ.'SFCSHP  '
     +       .OR.SUBSET(ITYP).EQ.'SALDRN  ') THEN
     ...
     $    .OR. SUBSET(ITYP).EQ.'GPSIPW  '.OR.SUBSET(ITYP).EQ.'WDSATR  '
     +    .OR. SUBSET(ITYP).EQ.'SALDRN  ')
...
C.......................................................................
     ...
     $                  subset.ne.'MSONET'.and.subset.ne.'SALDRN') then
...
C.......................................................................
     ...
     $                subset.eq.'MSONET'.or.subset.eq.'SALDRN') then
...
C$$$
     ...
     $ PROFILERinADPUPA,GOESCT,RASS,WRMISS,SKGP45,npkrpt,SKGNSS,UPRAIR
      CHARACTER*8  STNPRT
     ...
     $ RASS/.FALSE./,WRMISS/.TRUE./,SKGP45/.TRUE./,npkrpt/13*.false./
      DATA  SKGNSS/.TRUE./
      DATA  UPRAIR/.TRUE./
     ...
     $ FWINDO/4*300.,600.,8*300./,RWINDO/300./,UWINDO/300./,
     ...
     $ ISATOB/0/,IPRINT/0/,ISATLS/80*2/
      DATA  JAWIND/54*0/,JAMASS/54*0/,JSURFM/13*0/,
     $      JSURFW/2*0,9999,0,9999,0,9999,6*0/,
     ...
     $      TDRIFT_LL/MXLVL*XMISS/,KFLAG/5*0/,LFLAG/3*0/,GPSFLAG/2*0/,
